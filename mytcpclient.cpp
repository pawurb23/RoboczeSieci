...
myTCPclient::~myTCPclient() { gniazdo->disconnectFromHost(); }

void myTCPclient::polaczZSerwerem(const QString &ip, quint16 port) { gniazdo->connectToHost(ip, port); }
void myTCPclient::rozlaczZSerwerem() { gniazdo->disconnectFromHost(); }

void myTCPclient::wyslijDane(quint8 typ, const QByteArray &dane)
{
    if (gniazdo->state() != QAbstractSocket::ConnectedState) return;
...
void myTCPclient::obsluzPolaczenie() { emit polaczono(); }
void myTCPclient::obsluzRozlaczenie() { emit rozlaczono(); }
